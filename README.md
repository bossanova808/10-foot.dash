...
Unfortunately, with these clients, it is possible to 'back out of' the Jellyfin App in a way that 10-foot Dash can't detect - there is no 'media stop' event as such.  It seems like some of these clients auto-pause the media (others just keep playing in the background!) - but that's it.  It's a pretty crap but of Jellyfin 'design', really.
